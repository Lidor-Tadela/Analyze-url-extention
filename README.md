...
It isn't tied to any particular site — navigation is driven by a small
**rule system**. It ships with two disabled example rules; you add rules for
the sites you use without writing anything: open a chapter/episode page,
click the number in its address, done.

## Adding a site (no regex)

1. Open a chapter/episode page, e.g.
   `https://mangahost.example/series/some-series/chapter-5/`.
...
|---|---|
| `site/thing/chapter-N` | `^https://manga\.example/series/[^/]+/chapter-(\d+)/?$` |
| `site/thing/episode/N` | `^https://shows\.example/show/[^/]+/episode/(\d+)/?$` |
| `site/thing/N` | `^https://manga\.example/comic/[^/]+/(\d+)/?$` |
| number embedded mid-slug | `^https://shows\.example/watch/(\d+)-[^/]+$` |
